...
addInteractable(x, y, imageKey, hoverText, itemName, itemType, scale = 1) {
    ...
    item.on('pointerover', () => {
        hoverTextObject = this.add.text(
            x,
            y + item.displayHeight * 0.6,
            hoverText
        )
        .setOrigin(0.5, 0)
    ...
    item.on('pointerout', () => {
        if (hoverTextObject) {
            hoverTextObject.destroy();
        }
    });

        item.on('pointerdown', () => {

            this.showMessage("inspection text");

            if (item.inspected) return;
            item.inspected = true;

        
            let takeText = this.add.text(x, y + this.s * 2, "take " + itemName)
                .setFontSize(this.s * 1.5)
                .setFontStyle("bold")
                .setStroke("#000000", 5)
                ...
                .on('pointerdown', () => {
                    ...
                    if (this.hasType(itemType)) {
                        this.showMessage("text that plays if player already has one of item type");
                        return;
                    }

                    this.gainItem({ name: itemName, type: itemType });
            ...
            let leaveText = this.add.text(x, y + this.s * 4, "do not pick up")
                .setFontSize(this.s * 1.5)
                .setFontStyle("bold")
                .setStroke("#000000", 5)
...
//* logic base for making sure only one item of each color type can be picked up
    hasType(type) {
    ...
    updateInventory() {
        ...
        this.inventory.forEach((e, i) => {
            let text = this.add.text(this.w * 0.75 + 2 * this.s, h, e.name)//display inventory items name
     ...
     *
     * @param {string} item Item name. Must match the name passed to {@0nk AdventureScene#gainItem}.
     */
    loseItem(item) {
        if (!this.inventory.includes(item)) {
